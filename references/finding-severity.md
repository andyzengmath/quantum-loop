...
| medium   | A `testFirst: true` task has no `commands[]` entry matching `SPRINT_CONTRACT_TEST_REGEX` -- TDD intent without a test command. **OR** a single-story wave warning that masks a real bottleneck (warning is structurally correct but currently informational; operator could miss the underlying serialization risk). | `testfirst-no-test-command`: task says testFirst but only `commands: ["tsc --noEmit"]`. **OR** `single-story-wave-bottleneck-masked`: wave-1 = US-003 alone, where US-003 actually depends on a delayed library feature in US-001 -- dag-validator warning fires but doesn't block. |
