...
# v0.11.4 / US-002 (Path E split): Tests 14-18 (N46 respawn output re-parse
# + N43 dispatch_with_parallel_poll) moved to tests/test_dispatch_helpers.sh
# when this file crossed the architect's 600-LOC threshold at v0.11.1 ship.
# This file now stays focused on poll_orchestrator_commits +
# wrap_orchestrator_dispatch base/respawn/worktree behavior (Tests 1-13).

# (Tests 14-18 removed by Path E split; see tests/test_dispatch_helpers.sh)
: << 'PATHE_REMOVED_DOCSTRING'
Removed Tests:
  Test 14  → test_dispatch_helpers.sh::Test 1 (N46 respawn re-parse)
  Test 14c → test_dispatch_helpers.sh::Test 2 (set -e + pipefail under N46)
  Test 15  → test_dispatch_helpers.sh::Test 3 (graceful no-runner-parse-output)
  Test 16  → test_dispatch_helpers.sh::Test 4 (parallel-poll clean completion)
  Test 17  → test_dispatch_helpers.sh::Test 5 (parallel-poll STALE-kill)
  Test 18  → test_dispatch_helpers.sh::Test 6 (parallel-poll commit-progress reset)
PATHE_REMOVED_DOCSTRING
